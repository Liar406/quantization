...
model=../models/patch/Llama-2-7b-hf-quantization-singletask
task_dir=singletask
...
# tasks=arc_easy,winogrande,hellaswag,arc_challenge,boolq,piqa
tasks_array=(arc_easy)

for tasks in "${tasks_array[@]}"; do
    output_dir="${task_dir}_${tasks}"
    for bits in {5..5}; do
        echo $bits
        start=$(date +%s.%N)
